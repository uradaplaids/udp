...
#define FIRMWARE_VERSION 20251031
...
WiFiUDP wifiUdp;
Syslog syslog(wifiUdp, SYSLOGSERVER_SERVER, SYSLOGSERVER_PORT, HostName, "Ura da plaids", LOG_INFO);
#endif
      ...
      for (uint8_t i = 0; i < NUM_EVTS; i++)
      {
        ...
        if (settings.mySettings.events[i].enabled && (day() == day(settings.mySettings.events[i].time)) && (month() == month(settings.mySettings.events[i].time)) && !(minute() % rep_rate) && (second() < 30))
        {
          ...
          if (loadAnimation(String(settings.mySettings.events[i].animation)))
          {
            ...
            while ( millis() < (animationMillis + MIN_ANI_DURATION)) {
...
void handleButtonSettings()
{
  ...
  message += F("</head>");
  message += F("<body>");
  message += F("<h1>") + String(TXT_SETTINGS) + F("</h1>");
  message += F("</h1>");
...
void handleButtonEvents()
{
  ...
  message += F("<html>");
  message += F("<head>");
  message += F("<title>") + String(WEBSITE_TITLE) + F(" - ") + String(TXT_EVENT) + F("</title>");
  message += F("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">");
  message += F("<link rel=\"shortcut icon\" type=\"image/x-icon\" href=\"web/favicon.ico\">");
  message += F("<link rel=\"stylesheet\" href=\"https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css\">");
  message += F("<style>");
  ...
  message += F("</head>");
  message += F("<body>");
  message += F("<h1>") + String(TXT_EVENT) + F("</h1>");
  ...
  for (uint8_t i = 0; i < NUM_EVTS; i++) {
    ...
    message += String(settings.mySettings.events[i].txt) + F("\" pattern=\"[\\x20-\\x7e]{0,") + String(LEN_EVT_STR - 1) + F("}\" placeholder=\"text eveniment ...\">");
    message += F("</td></tr>");
    message += F("<tr><td>");
...
void handleCommitEvents()
{
  ...
  message += F("<meta charset=\"UTF-8\">");
  message += F("<meta http-equiv=\"refresh\" content=\"3;url=/\">");
  message += F("<title>Gespeichert</title>");
  message += F("<style>body{font-family:Sans-serif;text-align:center;padding-top:50px;color:#333;}");
  message += F("h2{color:#1FA3EC;} p{font-size:18px;}</style>");
  message += F("</head><body>");
  message += F("<h2>✅ ") + String(TXT_EVENT_CONFIRM) + F("</h2>");
  message += F("<p>") + String(TXT_EVENT_REDIRECT) + F("</p>");
  message += F("</body></html>");
...
void handleCommitAdminSettings()
{
  ...
  currentFrontcover = settings.mySettings.frontCover;
